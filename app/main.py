...
async def startup_event():
    ...
    try:
        ...
        try:
            [IP_LIST, NODE_LIST] = await list_devices(RADIO_IP)
...
async def log_in(credentials: Credentials):
    ...
    try:
        ...
        res = await api_login(username, pw, RADIO_IP)
        if res:
            msg = "Success"
...
async def net_data():
    ...
    if IP_LIST and NODE_LIST:
        ...
        snrs = await net_status(RADIO_IP, NODE_LIST[:-1]) # N-1 queries is enough to know all SNRs
...
async def basic_settings(settings: Optional[BasicSettings] = None):
    ...
    try:
        if settings:
            response = await set_basic.set_basic_settings(RADIO_IP, NODE_LIST, settings)
            msg = {"Error"} if "error" in response else {"Success"}

            return msg
        else:
            response = await get_basic_set(RADIO_IP)
            return response
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def net_stat():
    ...
    res = await net_status(RADIO_IP, NODE_LIST)
    return res
...
async def get_battery():
    ...
    """
    global IP_LIST
    ips_batteries = await get_batteries(IP_LIST)
...
async def get_camera():
    ...
    global IP_LIST
    try:
        streams = await find_camera_streams_temp(IP_LIST)
