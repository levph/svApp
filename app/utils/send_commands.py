...
async def api_login(un, pw, radio_ip):
    """
    Login function
    """
    global COOKIE
    ...
    if response.status_code == 200:
        try:
            ...
            res = await send_commands_ip(["routing_tree"], radio_ip=radio_ip, params=[[]])
...
async def send_net_stat(radio_ip, nodelist):
    global COOKIE 
    ...
    async with lock:
        response = await asyncio.to_thread(requests.post, url, headers=headers, data=payload, timeout=10, cookies = COOKIE)
        if response.status_code == 200:
            COOKIE = response.cookies
        return response
...
async def send_commands_ip(methods, radio_ip, params=None, timeout=None):
    """
    ...
    async with lock:
        try:
            response = await asyncio.to_thread(requests.post, api_endpoint, payload, timeout=10, cookies = COOKIE)
